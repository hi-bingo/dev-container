...
- `compose.yaml` 会挂载当前工作目录到 `/workspace`，并把宿主机 `${HOME}/.ssh` 挂到容器内 `/home/dev/.ssh`
- `compose.yaml` 会把宿主机 `${HOME}/.gitconfig` 挂到容器内 `/home/dev/.gitconfig`，直接复用主机上的 Git 用户名、邮箱和别名配置
- `compose.yaml` 也会把宿主机 `${HOME}/.codex` 和 `${HOME}/.claude` 挂进容器，直接复用主机上的 Codex / Claude Code 配置与登录态
